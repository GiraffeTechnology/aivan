...
4. Rerun the six exact B1-A regression nodeids, full pytest, Ruff, Mypy, Bandit,
   module budget, and Stage 6 five-pass preflight on the revert candidate.
...
mutations without a real dependency probe. Therefore a reverted build is not a
production-safe acceptance path and must remain blocked from deployment until
an independently accepted replacement exists.
