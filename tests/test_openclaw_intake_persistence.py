...
@pytest.mark.asyncio
async def test_debug_endpoint_lists_intake_sheets(client, db_override):
    await client.post(
        "/api/openclaw/events",
        json={
            "source": "openclaw",
    ...
    response = await client.get("/api/intake/inquiry-sheets")

    assert response.status_code == 200
    sheets = response.json()["inquiry_sheets"]
    assert len(sheets) == 1
    assert sheets[0]["message_count"] == 1
    assert sheets[0]["messages"][0]["structured_json"]["destination"] == "东京"
