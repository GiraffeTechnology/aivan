...
The ✉️ action sends email through aivan-openclaw's controlled real-test
transport when it is configured. When it is not, a feature-flagged mock
...
class EmailSendResult:
    success: bool
    provider: str  # "aivan-openclaw" | "mock" | "none"
    message_id: str = ""
    error: str = ""
...
def send_email(*, to: str, subject: str, body: str, case_id: str, draft_id: str) -> EmailSendResult:
    ...
    if status == "configured":
        # Reuse the approval-gated aivan-openclaw transport. It re-validates
        # the recipient against AIVAN_EMAIL_ALLOWED_RECIPIENTS and never
        # reports success unless SMTP accepted the message.
