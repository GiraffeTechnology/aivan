...
class PlatformRepository:
    ...
    def upsert(self, platform_id: str, data: dict, *, tenant_id: str = "legacy") -> PlatformRecord:
        existing = self.db.query(PlatformRecord).filter(PlatformRecord.platform_id == platform_id, PlatformRecord.tenant_id == tenant_id).first()
        if existing:
            ...
            self.db.flush()
            return existing
        record = PlatformRecord(platform_id=platform_id, tenant_id=tenant_id, **{k: v for k, v in data.items() if hasattr(PlatformRecord, k) and k not in {"platform_id", "tenant_id"}})
        self.db.add(record)
        self.db.flush()
        return record

    def get(self, platform_id: str, *, tenant_id: str = "legacy") -> PlatformRecord | None:
        return self.db.query(PlatformRecord).filter(PlatformRecord.platform_id == platform_id, PlatformRecord.tenant_id == tenant_id).first()
