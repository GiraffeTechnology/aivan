...
@app.get("/api/intake/inquiry-sheets")
def list_inquiry_sheets(limit: int = 50, db: Session = Depends(get_db)):
    from aivan.db.models.intake import InquirySheet
    from aivan.intake.persistence import serialize_sheet

    sheets = (
        db.query(InquirySheet)
        .order_by(InquirySheet.created_at.desc())
        .limit(min(max(limit, 1), 200))
        .all()
    )
    return {"inquiry_sheets": [serialize_sheet(sheet) for sheet in sheets]}


@app.get("/api/intake/inquiry-sheets/{sheet_id}")
def get_inquiry_sheet(sheet_id: str, db: Session = Depends(get_db)):
    from aivan.db.models.intake import InquirySheet
    from aivan.intake.persistence import serialize_sheet

    sheet = db.query(InquirySheet).filter(InquirySheet.id == sheet_id).first()
    if not sheet:
        raise HTTPException(status_code=404, detail="Inquiry sheet not found")
