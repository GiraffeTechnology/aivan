...
from aiven.platforms.platform_registry import is_platform_trusted, is_platform_blocked, suggest_platform
from aiven.platforms.domain_utils import normalize_domain, detect_typosquatting, detect_punycode_or_homograph
...
def check_and_suggest_domain(domain: str, reason: str = "found in supplier discovery") -> dict:
    ...
    for tid in ["alibaba", "aliexpress"]:
        from aiven.platforms.platform_registry import get_platform
        p = get_platform(tid)
        if p:
            from aiven.platforms.domain_utils import is_domain_allowed
