...
from aiven.db.session import get_db, init_db
...
app = FastAPI(title="AIVEN - AI Trade Salesperson", version="0.1.0", lifespan=lifespan)
...
@app.get("/health")
def health():
    return {"status": "ok", "product": "AIVEN", "version": "0.1.0"}

@app.get("/app", response_class=HTMLResponse)
@app.get("/", response_class=HTMLResponse)
def serve_app(request: Request):
    return templates.TemplateResponse("index.html", {"request": request, "title": "AIVEN"})
...
def approve_draft(draft_id: str, body: dict = None, db: Session = Depends(get_db)):
    ...
    from aiven.openclaw.outbound_approval import send_if_approved
...
def import_suppliers(body: dict, db: Session = Depends(get_db)):
    ...
    from aiven.sourcing.supplier_importer import import_from_csv
...
def list_platforms():
    from aiven.platforms.platform_registry import list_all_platforms
    platforms = list_all_platforms()
    return {"platforms": [p.model_dump() for p in platforms]}

@app.get("/api/platforms/whitelist")
def list_whitelist():
    from aiven.platforms.platform_registry import list_trusted_platforms
...
def add_platform_to_whitelist(body: dict, db: Session = Depends(get_db)):
    ...
    from aiven.platforms.platform_registry import add_platform
...
def list_platform_suggestions():
    from aiven.platforms.platform_registry import list_suggestions
...
def approve_platform_suggestion(suggestion_id: str):
    from aiven.platforms.platform_registry import approve_suggestion
...
def reject_platform_suggestion(suggestion_id: str):
    from aiven.platforms.platform_registry import reject_suggestion
...
def block_platform_suggestion(suggestion_id: str):
    from aiven.platforms.platform_registry import block_suggestion
