...
def cmd_init(args):
    print("Initializing AIVEN...")
    os.makedirs("data", exist_ok=True)
    from aiven.db.session import init_db
    init_db()
    ...
    print("AIVEN ready. Run: uv run aiven serve")
...
def cmd_import_suppliers(args):
    ...
    if not path or not os.path.exists(path):
        ...
        sys.exit(1)
    from aiven.sourcing.supplier_importer import import_from_csv_file
    from aiven.db.session import db_session
...
def cmd_demo(args):
    print("=" * 60)
    print("AIVEN TRADE SALESPERSON DEMO")
    print("=" * 60)
    from aiven.db.session import db_session, init_db
    init_db()
    ...
    print("\n" + "=" * 60)
    print("AIVEN TRADE SALESPERSON E2E: PASS")
    print("=" * 60)

def cmd_demo_marketplace(args):
    print("=" * 60)
    print("AIVEN MARKETPLACE SOURCING DEMO")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("AIVEN MARKETPLACE SOURCING E2E: PASS")
    print("=" * 60)

def cmd_demo_risk_check(args):
    print("=" * 60)
    print("AIVEN RISK CHECK DEMO")
    print("=" * 60)
    from aiven.risk.supplier_risk_agent import run_risk_screening
    from aiven.risk.risk_report import format_risk_report_text
...
def cmd_platforms(args):
    ...
    if sub == "list":
        from aiven.platforms.platform_registry import list_all_platforms, _ensure_init
        _ensure_init()
        for p in list_all_platforms():
            print(f"  [{p.status}] {p.platform_id}: {p.display_name} | Domains: {p.domain_patterns}")
    elif sub == "whitelist":
        from aiven.platforms.platform_registry import list_trusted_platforms, _ensure_init
        _ensure_init()
        for p in list_trusted_platforms():
            print(f"  {p.platform_id}: {p.display_name}")
    elif sub == "suggest":
        from aiven.platforms.platform_registry import suggest_platform, _ensure_init
        _ensure_init()
...
def cmd_accounts(args):
    ...
    init_db()
    if sub == "list":
        from aiven.openclaw.account_delegation import list_accounts
        with db_session() as db:
        ...
        with open(path) as f:
            data = json.load(f)
        from aiven.openclaw.account_delegation import register_account
        with db_session() as db:
        ...
        if not account_id:
            ...
            sys.exit(1)
        from aiven.openclaw.account_delegation import revoke_account
        with db_session() as db:
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="aiven", description="AIVEN - AI Trade Salesperson CLI")
    subparsers = parser.add_subparsers(dest="command")

    subparsers.add_parser("init", help="Initialize AIVEN database and settings")
    serve_p = subparsers.add_parser("serve", help="Start AIVEN local web server")
    subparsers.add_parser("demo", help="Run core AIVEN E2E demo")
